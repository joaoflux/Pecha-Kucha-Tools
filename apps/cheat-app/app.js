...
function displayImage(num){
	if (num < 21) {
		...
		var notefile0 = notes[num-1].file;
		var thumbnail0 = "<img class='thumbnail' src='"+file0+"'>";
		var note0 = $('<div id="current-notes">');
		note0.load(notefile0, function(){
    		// now $(this) contains file content
		});
		var img0 = thumbnail0;
	} else {
			var img0 = "";
	...
	if (num < 20) {
		...
		var thumbnail1 = "<img class='thumbnail' src='"+file1+"'>";
		var img1 = thumbnail1;
	}else {
		var img1 = "";
	}
   	if(num == 0){
		...
		$('#current-notes').replaceWith(note0);
   		$('#next-slide').html(img1);
